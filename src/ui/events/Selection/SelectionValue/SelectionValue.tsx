...
const SelectionValue = ({ valueRef, displayPrice, isLocked, isSuspended, isDisplay = true }: Props) => {
    const {
    ...
    if (isTabletSmall) {
        return <S_SelectionValue>{selectionValue}</S_SelectionValue>;
    }

    return (
        <S_SelectionValueIdentifier>
            <S_SelectionValue ref={valueRef} isSuspended={isSuspended} data-testid='selectionPrice'>
                {selectionValue}
            </S_SelectionValue>
        </S_SelectionValueIdentifier>
    );
